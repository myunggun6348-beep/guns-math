...
module.exports = async (req, res) => {
  ...
  try {
    ...
    for (const key of keys) {
      ...
      students.push({
        id, name: account.name || "", grade: String(data.profile?.grade || data.pref?.grade || ""), track: String(data.profile?.track || data.pref?.track || ""),
        ...
        activeWrong: notes.filter(note => !note.mastered).length, mastered: notes.filter(note => note.mastered).length,
