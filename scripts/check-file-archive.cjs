...
(async()=>{
 ...
 try{for(const viewport of [{width:1200,height:900},{width:390,height:844}]){const page=await browser.newPage({viewport});await page.goto("http://127.0.0.1:5196/files.html");await page.waitForSelector(".archive-card");assert.equal(await page.locator(".grade-tab").count(),5);assert.equal(await page.locator(".archive-card").count(),4);await page.click('[data-grade="3"]');assert.equal(await page.locator(".archive-card").count(),1);assert.equal(await page.locator("#archiveHeading").textContent(),"3학년");assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),false);await page.close()}}
