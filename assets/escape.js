(() => {
  ...
  async function openQuestion(question) {
    ...
    if (question.format === "choice") { const list = document.createElement("div"); list.className = "choice-list"; question.choices.forEach((choice, index) => { const label = document.createElement("label"), input = document.createElement("input"); input.type = "radio"; input.name = "choice"; input.value = choice.id; label.append(input, document.createTextNode(` ${index + 1}. ${choice.label}`)); list.append(label); }); area.append(list); }
  ...
  $("#hintButton").addEventListener("click", async () => { try { const data = await call("hint", { id: run.id, playerKey, question: currentQuestion.id }); run = data.run; currentQuestion = run.questions[currentQuestion.id]; $("#hintText").textContent = currentQuestion.hint; $("#hintButton").hidden = true; $("#score").textContent = run.score; } catch (error) { errorMessage(error); } });
  $("#submitAnswer").addEventListener("click", async () => {
    ...
    try { const data = await call("answer", { id: run.id, playerKey, question: currentQuestion.id, answer }); run = data.run; const solved = run.solved.includes(currentQuestion.id); $("#answerMessage").textContent = data.feedback; $("#answerMessage").className = `answer-message ${solved ? "good" : "bad"}`; if (solved) { $("#submitAnswer").disabled = true; setTimeout(() => { $("#questionDialog").close(); renderGame(); }, 750); } else $("#score").textContent = run.score; } catch (error) { errorMessage(error); }
  });
