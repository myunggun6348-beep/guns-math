(function(){
  ...
  function setup(seed=profile){
    const grade=seed?.grade||"1",name=seed?.name||"",available=Object.entries(tracks).filter(([,track])=>track.grades.includes(grade));
    root.hidden=false;root.innerHTML='<div class="home-personal-shell setup"><div class="home-personal-intro"><span class="eyebrow">MY HOME</span><h2>내 학습 홈 만들기</h2><p>학년과 과목을 한 번 정하면 다음 방문부터 오늘 할 공부와 복습할 내용을 바로 보여 줍니다.</p></div><form id="homeProfileForm" class="home-profile-form"><label>이름 또는 별명 <input name="name" maxlength="20" value="'+esc(name)+'" placeholder="선택 사항"></label><label>학년 <select name="grade"><option value="1">고1</option><option value="2">고2</option><option value="3">고3</option></select></label><label>주로 공부할 과목 <select name="track"></select></label><button class="btn btn-primary" type="submit">내 학습 홈 시작 →</button></form></div>';
    ...
    form.addEventListener("submit",event=>{event.preventDefault();save({name:form.elements.name.value,grade:gradeSelect.value,track:trackSelect.value})});
  }
  function render(){
    ...
    root.hidden=false;root.innerHTML='<div class="home-personal-shell"><div class="home-personal-head"><div><span class="eyebrow">MY HOME</span><h2>'+greeting+'</h2><p>고'+profile.grade+' · '+esc(track.title)+' 기준으로 오늘 필요한 학습을 모았습니다.</p></div><button type="button" id="homeProfileEdit">학년·과목 변경</button></div><div class="home-personal-grid">'+
