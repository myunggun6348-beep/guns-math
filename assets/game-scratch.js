(function(){
  "use strict";
  const pages=new Map();
  let observer=null,escapeHandler=null;
  let settings={tool:"pen",color:"#172033",size:4,finger:false};

  function markup(){
    return '<section class="game-scratch" aria-label="문제 풀이 공간"><div class="scratch-heading"><div><strong>풀이 공간</strong><span>태블릿 펜으로 바로 계산해 보세요.</span></div><div class="scratch-heading-actions"><span class="scratch-hint">손바닥과 손가락은 화면 이동</span><button type="button" class="scratch-expand" id="scratchExpand" aria-expanded="false">⛶ 전체 화면</button></div></div><div class="scratch-toolbar" role="toolbar" aria-label="필기 도구"><div class="scratch-tool-group"><button type="button" class="scratch-tool active" data-scratch-tool="pen" aria-pressed="true">✎ 펜</button><button type="button" class="scratch-tool" data-scratch-tool="eraser" aria-pressed="false">지우개</button></div><div class="scratch-tool-group scratch-colors" aria-label="펜 색상"><button type="button" class="scratch-color active" data-scratch-color="#172033" aria-label="검정색" aria-pressed="true"></button><button type="button" class="scratch-color blue" data-scratch-color="#2563eb" aria-label="파란색" aria-pressed="false"></button><button type="button" class="scratch-color red" data-scratch-color="#dc2626" aria-label="빨간색" aria-pressed="false"></button></div><label class="scratch-size">굵기 <input type="range" id="scratchSize" min="2" max="10" step="2" value="4" aria-label="펜 굵기"></label><div class="scratch-tool-group"><button type="button" class="scratch-icon" id="scratchUndo" aria-label="되돌리기" title="되돌리기">↶</button><button type="button" class="scratch-icon" id="scratchRedo" aria-label="다시 실행" title="다시 실행">↷</button><button type="button" class="scratch-clear" id="scratchClear">전체 지우기</button></div><label class="scratch-finger"><input type="checkbox" id="scratchFinger"> 손가락 필기</label></div><div class="scratch-paper"><canvas id="scratchCanvas" aria-label="필기용 풀이판"></canvas><span class="scratch-placeholder">식을 세우고 계산 과정을 적어 보세요</span></div></section>';
  }
  function pageFor(key){if(!pages.has(key))pages.set(key,{strokes:[],undo:[],redo:[]});return pages.get(key)}
  function capture(key){
    const page=pageFor(key);if(!page.strokes.length)return "";
    const w=900,maxY=Math.max(...page.strokes.flatMap(s=>s.points.map(p=>p.y)),0),h=Math.max(320,Math.min(1600,Math.ceil(maxY*w+30))),out=document.createElement("canvas");out.width=w;out.height=h;
    const c=out.getContext("2d");c.fillStyle="#fff";c.fillRect(0,0,w,h);c.strokeStyle="rgba(77,111,171,.14)";c.lineWidth=1;
    ...
    page.strokes.forEach(stroke=>{if(!stroke.points.length)return;c.save();c.lineCap="round";c.lineJoin="round";c.strokeStyle=stroke.color;c.lineWidth=stroke.width;c.beginPath();const first=stroke.points[0];c.moveTo(first.x*w,first.y*w);if(stroke.points.length===1)c.lineTo(first.x*w+.01,first.y*w+.01);for(let i=1;i<stroke.points.length;i++){const p=stroke.points[i];c.lineTo(p.x*w,p.y*w)}c.stroke();c.restore()});
    return out.toDataURL("image/webp",.76);
  }
  function mount(key){
    clean()
    const canvas=document.getElementById("scratchCanvas");if(!canvas)return;
    const paper=canvas.parentElement,page=pageFor(key),ctx=canvas.getContext("2d"),undoBtn=document.getElementById("scratchUndo"),redoBtn=document.getElementById("scratchRedo"),clearBtn=document.getElementById("scratchClear"),sizeInput=document.getElementById("scratchSize"),fingerInput=document.getElementById("scratchFinger"),expandBtn=document.getElementById("scratchExpand"),shell=canvas.closest(".game-scratch");
    let drawing=false,changed=false,before=null,activeStroke=null,scrollPointer=null;
    ...
    function line(stroke){if(!stroke.points.length)return;ctx.save();ctx.lineCap="round";ctx.lineJoin="round";ctx.strokeStyle=stroke.color;ctx.lineWidth=stroke.width;ctx.beginPath();const first=stroke.points[0];ctx.moveTo(first.x*canvas.clientWidth,first.y*canvas.clientWidth);if(stroke.points.length===1)ctx.lineTo(first.x*canvas.clientWidth+.01,first.y*canvas.clientWidth+.01);for(let i=1;i<stroke.points.length;i++){const p=stroke.points[i];ctx.lineTo(p.x*canvas.clientWidth,p.y*canvas.clientWidth)}ctx.stroke();ctx.restore()}
    function redraw(){ctx.clearRect(0,0,canvas.clientWidth,canvas.clientHeight);page.strokes.forEach(line);updateState()}
    function point(e){const r=canvas.getBoundingClientRect();return{x:(e.clientX-r.left)/r.width,y:(e.clientY-r.top)/r.width}}
    function distanceToSegment(p,a,b){const w=canvas.clientWidth,px=p.x*w,py=p.y*w,ax=a.x*w,ay=a.y*w,bx=b.x*w,by=b.y*w,dx=bx-ax,dy=by-ay,l=dx*dx+dy*dy,t=l?Math.max(0,Math.min(1,((px-ax)*dx+(py-ay)*dy)/l)):0;return Math.hypot(px-(ax+t*dx),py-(ay+t*dy))}
    ...
    function down(e){if(!allowed(e)){if(e.pointerType==="touch"&&!drawing){e.preventDefault();canvas.setPointerCapture(e.pointerId);scrollPointer={id:e.pointerId,y:e.clientY}}return}e.preventDefault();canvas.setPointerCapture(e.pointerId);drawing=true;changed=false;before=clone(page.strokes);const p=point(e);if(settings.tool==="eraser")eraseAt(p);else{activeStroke={color:settings.color,width:settings.size,points:[p]};page.strokes.push(activeStroke);changed=true;redraw()}}
    function move(e){if(scrollPointer&&scrollPointer.id===e.pointerId){e.preventDefault();const dy=e.clientY-scrollPointer.y;window.scrollBy(0,-dy);scrollPointer.y=e.clientY;return}if(!drawing||!allowed(e))return;e.preventDefault();const p=point(e);if(settings.tool==="eraser")eraseAt(p);else{activeStroke.points.push(p);redraw()}}
    function up(e){if(scrollPointer&&scrollPointer.id===e.pointerId){e.preventDefault();scrollPointer=null;return}if(!drawing)return;if(allowed(e))e.preventDefault();drawing=false;activeStroke=null;if(changed){page.undo.push(before);if(page.undo.length>30)page.undo.shift();page.redo=[]}updateState()}
    ...
    function setExpanded(on){shell.classList.toggle("scratch-expanded",on);document.body.classList.toggle("scratch-open",on);expandBtn.setAttribute("aria-expanded",String(on));expandBtn.textContent=on?"↙ 원래 크기":"⛶ 전체 화면";requestAnimationFrame(fit)}
    expandBtn.addEventListener("click",()=>setExpanded(!shell.classList.contains("scratch-expanded")));escapeHandler=e=>{if(e.key==="Escape"&&shell.classList.contains("scratch-expanded"))setExpanded(false)};document.addEventListener("keydown",escapeHandler);
  ...
  function clean(){if(observer){observer.disconnect();observer=null}if(escapeHandler){document.removeEventListener("keydown",escapeHandler);escapeHandler=null}document.body.classList.remove("scratch-open")}
  function reset(){pages.clear();clean()}
  function destroy(){clean()}
  window.GameScratch={markup,mount,reset,destroy,capture};
})();
